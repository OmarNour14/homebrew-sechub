...
case "$1" in
    add-note)
        ...
        ;;
    download-report)
        FILE_NAME="FindingsReport.xlsx"
        BUCKET_PATH="s3://tf-security-hub-reports-fdrpsyqv/WeeklyReports/$FILE_NAME"
        LOCAL_PATH=${2:-"./$FILE_NAME"}

        # Download the report
        echo "Downloading report to $LOCAL_PATH..."
        aws s3 cp "$BUCKET_PATH" "$LOCAL_PATH" && echo "Download complete." || echo "Failed to download the report."
        ;;
    *)
